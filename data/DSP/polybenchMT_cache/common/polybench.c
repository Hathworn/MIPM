...
void polybench_timer_start(double *time_start)
{
...
void polybench_timer_stop(double *time_end)
{
#ifndef POLYBENCH_CYCLE_ACCURATE_TIMER
    *time_end = rtclock();
#else
...
void polybench_timer_print(double *time_start, double *time_end)
{
    ...
    if (__polybench_program_total_flops == 0) {
        ...
        printf("%0.2lf\n",
               (__polybench_program_total_flops / (double)(polybench_t_end - polybench_t_start)) / 1000000000);
#else
#ifndef POLYBENCH_CYCLE_ACCURATE_TIMER
    printf("%0.6f\n", *time_end - *time_start);
#else
...
void *polybench_alloc_data(unsigned long long int n, int elt_size, int cluster_id, int mode)
{
    /// FIXME: detect overflow!
    size_t val = n;
    val *= elt_size;
    void *ret = hthread_malloc(cluster_id, val, mode);
