...
#define POLYBENCH_FREE_ARRAY(x) hthread_free((void *)x);
...
/* Macros to allocate heap arrays with cluster_id and mode parameters
   Example:
   polybench_alloc_2d_array(N, M, double) => allocates N x M x sizeof(double)
                      and returns a pointer to the 2d array
 */
#define POLYBENCH_ALLOC_1D_ARRAY(n1, type, cluster_id, mode)                                                           \
    (type(*)[n1 + POLYBENCH_PADDING_FACTOR])                                                                           \
        polybench_alloc_data(n1 + POLYBENCH_PADDING_FACTOR, sizeof(type), cluster_id, mode)

#define POLYBENCH_ALLOC_2D_ARRAY(n1, n2, type, cluster_id, mode)                                                       \
    (type(*)[n1 + POLYBENCH_PADDING_FACTOR][n2 + POLYBENCH_PADDING_FACTOR]) polybench_alloc_data(                      \
        (n1 + POLYBENCH_PADDING_FACTOR) * (n2 + POLYBENCH_PADDING_FACTOR), sizeof(type), cluster_id, mode)

#define POLYBENCH_ALLOC_3D_ARRAY(n1, n2, n3, type, cluster_id, mode)                                                   \
    (type(*)[n1 + POLYBENCH_PADDING_FACTOR][n2 + POLYBENCH_PADDING_FACTOR][n3 + POLYBENCH_PADDING_FACTOR])             \
        polybench_alloc_data((n1 + POLYBENCH_PADDING_FACTOR) * (n2 + POLYBENCH_PADDING_FACTOR) *                       \
                             ...
                             sizeof(type), cluster_id, mode)

#define POLYBENCH_ALLOC_4D_ARRAY(n1, n2, n3, n4, type, cluster_id, mode)                                               \
    (type(*)[n1 + POLYBENCH_PADDING_FACTOR][n2 + POLYBENCH_PADDING_FACTOR][n3 + POLYBENCH_PADDING_FACTOR]              \
...
#define POLYBENCH_ALLOC_5D_ARRAY(n1, n2, n3, n4, n5, type, cluster_id, mode)                                           \
    (type(*)[n1 + POLYBENCH_PADDING_FACTOR][n2 + POLYBENCH_PADDING_FACTOR][n3 + POLYBENCH_PADDING_FACTOR]              \
...
#ifndef POLYBENCH_STACK_ARRAYS
#define POLYBENCH_1D_ARRAY_DECL(var, type, dim1, ddim1, cluster_id, mode)                                              \
    type POLYBENCH_1D(POLYBENCH_DECL_VAR(var), dim1, ddim1);                                                           \
    var = POLYBENCH_ALLOC_1D_ARRAY(POLYBENCH_C99_SELECT(dim1, ddim1), type, cluster_id, mode);

#define POLYBENCH_2D_ARRAY_DECL(var, type, dim1, dim2, ddim1, ddim2, cluster_id, mode)                                 \
...
#define POLYBENCH_3D_ARRAY_DECL(var, type, dim1, dim2, dim3, ddim1, ddim2, ddim3, cluster_id, mode)                    \
    ...
    var = POLYBENCH_ALLOC_3D_ARRAY(POLYBENCH_C99_SELECT(dim1, ddim1), POLYBENCH_C99_SELECT(dim2, ddim2),               \
                                   POLYBENCH_C99_SELECT(dim3, ddim3), type, cluster_id, mode);

#define POLYBENCH_4D_ARRAY_DECL(var, type, dim1, dim2, dim3, dim4, ddim1, ddim2, ddim3, ddim4, cluster_id, mode)       \
...
#define POLYBENCH_5D_ARRAY_DECL(var, type, dim1, dim2, dim3, dim4, dim5, ddim1, ddim2, ddim3, ddim4, ddim5,            \
    ...
    var = POLYBENCH_ALLOC_5D_ARRAY(POLYBENCH_C99_SELECT(dim1, ddim1), POLYBENCH_C99_SELECT(dim2, ddim2),               \
                                   ...
                                   POLYBENCH_C99_SELECT(dim5, ddim5), type, cluster_id, mode);
#else
#define POLYBENCH_1D_ARRAY_DECL(var, type, dim1, ddim1) type POLYBENCH_1D(POLYBENCH_DECL_VAR(var), dim1, ddim1);
...
/* Function prototypes. */
extern void *polybench_alloc_data(unsigned long long int n, int elt_size, int cluster_id, int mode);
