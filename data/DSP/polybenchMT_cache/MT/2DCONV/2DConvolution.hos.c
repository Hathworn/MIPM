...
void compareResults(int ni, int nj, DATA_TYPE POLYBENCH_2D(B, NI, NJ, ni, nj),
                    DATA_TYPE POLYBENCH_2D(B_outputFromDsp, NI, NJ, ni, nj))
{
    int i, j, fail;
    fail = 0;
...
void convolution2DDSP(int ni, int nj, DATA_TYPE POLYBENCH_2D(A, NI, NJ, ni, nj),
...
void convolution2DDSP_cache(int ni, int nj, DATA_TYPE POLYBENCH_2D(A, NI, NJ, ni, nj),
                            DATA_TYPE POLYBENCH_2D(B, NI, NJ, ni, nj),
                            DATA_TYPE POLYBENCH_2D(B_outputFromDsp, NI, NJ, ni, nj), double *t_DSP_start,
                            double *t_DSP_end)
{
    memcpy(B_outputFromDsp, B, sizeof(DATA_TYPE) * ni * nj);
    ...
    // 创建线程组并运行
    int groupId = hthread_group_create(clusterId, nthreads, kernel_cache, 2, 2, args);
    // 验证线程组创建是否成功
    if (groupId == -1) {
        M_logError("Failed to create threads with %s", kernel_cache);
        return;
    }
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < ROUND; i++) {
        ...
        convolution2DDSP(ni, nj, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(B_outputFromDsp), &t_DSP_start,
                         &t_DSP_end);
        t_DSP_sum += t_DSP_end - t_DSP_start;

        init(ni, nj, POLYBENCH_ARRAY(A));
        convolution2DDSP_cache(ni, nj, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(B_outputFromDsp),
    ...
    polybench_PM_print_cache(kernel, clusterId, nthreads, t_CPU_sum / ROUND, t_DSP_sum / ROUND, t_DSP_cache_sum / ROUND,
                             workload_info);
