...
void fdtd2dDSP_cache(int tmax, int nx, int ny, DATA_TYPE POLYBENCH_1D(_fict_, TMAX, TMAX), DATA_TYPE POLYBENCH_2D(ex, NX, NY, nx, ny),
               DATA_TYPE POLYBENCH_2D(ey, NX, NY, nx, ny), DATA_TYPE POLYBENCH_2D(hz, NX, NY, nx, ny), DATA_TYPE POLYBENCH_2D(hz_outputFromDSP, NX, NY, nx, ny), double *t_DSP_start, double *t_DSP_end) {
    memcpy(hz_outputFromDSP, hz, sizeof(DATA_TYPE) * NX * NY);
    ...
    for (int t = 0; t < _PB_TMAX; t++) {
        args1[2] = (uint64_t)toRawBits(t); // 每次迭代更新i1
        args2[2] = (uint64_t)toRawBits(t); // 每次迭代更新i1
        hthread_group_exec(groupId, kernel1_cache, 3, 4, args2);
        hthread_group_wait(groupId);
        hthread_group_exec(groupId, kernel2_cache, 3, 3, args1);
        hthread_group_wait(groupId);
        hthread_group_exec(groupId, kernel3_cache, 3, 3, args1);
        hthread_group_wait(groupId);
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < ROUND; i++) {
        ...
        fdtd2dDSP_cache(tmax, nx, ny, POLYBENCH_ARRAY(_fict_), POLYBENCH_ARRAY(ex), POLYBENCH_ARRAY(ey), POLYBENCH_ARRAY(hz), POLYBENCH_ARRAY(hz_outputFromDsp), &t_DSP_start, &t_DSP_end);
    ...
    polybench_PM_print_cache("fdtd2d", clusterId, nthreads, t_CPU_sum / ROUND, t_DSP_sum / ROUND, t_DSP_cache_sum / ROUND, get_dataset_name());
