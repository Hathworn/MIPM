...
void init_arrays(int ni, int nj, DATA_TYPE *alpha, DATA_TYPE *beta, DATA_TYPE POLYBENCH_2D(A, NI, NJ, ni, nj),
                 DATA_TYPE POLYBENCH_2D(B, NI, NJ, ni, nj), DATA_TYPE POLYBENCH_2D(C, NI, NI, ni, ni))
{
    int i, j;
...
void syr2kCpu(int ni, int nj, DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE POLYBENCH_2D(A, NI, NJ, ni, nj),
              DATA_TYPE POLYBENCH_2D(B, NI, NJ, ni, nj), DATA_TYPE POLYBENCH_2D(C, NI, NI, ni, ni))
{
    int i, j, k;
...
void compareResults(int ni, DATA_TYPE POLYBENCH_2D(C, NI, NI, ni, ni),
                    DATA_TYPE POLYBENCH_2D(C_outputFromDsp, NI, NI, ni, ni))
{
    int i, j, fail;
    fail = 0;
...
void syr2kDsp(int ni, int nj, DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE POLYBENCH_2D(A, NI, NJ, ni, nj),
              DATA_TYPE POLYBENCH_2D(B, NI, NJ, ni, nj), DATA_TYPE POLYBENCH_2D(C, NI, NI, ni, ni),
              DATA_TYPE POLYBENCH_2D(C_outputFromDsp, NI, NI, ni, ni), double *t_DSP_start, double *t_DSP_end)
{
    ...
    int groupId = hthread_group_create(clusterId, nthreads, kernel, 4, 3, args);
...
void syr2kDsp_cache(int ni, int nj, DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE POLYBENCH_2D(A, NI, NJ, ni, nj),
              DATA_TYPE POLYBENCH_2D(B, NI, NJ, ni, nj), DATA_TYPE POLYBENCH_2D(C, NI, NI, ni, ni),
              DATA_TYPE POLYBENCH_2D(C_outputFromDsp, NI, NI, ni, ni), double *t_DSP_start, double *t_DSP_end)
{
    memcpy(C_outputFromDsp, C, sizeof(DATA_TYPE) * ni * ni);
    ...
    int groupId = hthread_group_create(clusterId, nthreads, kernel_cache, 4, 3, args);
    hthread_group_wait(groupId);
...
int main()
{
    ...
    for (int i = 0; i < ROUND; i++) {
        ...
        syr2kDsp(ni, nj, alpha, beta, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(C),
                 POLYBENCH_ARRAY(C_outputFromDsp), &t_DSP_start, &t_DSP_end);
        t_DSP_sum += t_DSP_end - t_DSP_start;

        init_arrays(ni, nj, &alpha, &beta, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(C));
        syr2kDsp_cache(ni, nj, alpha, beta, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(C),
                 POLYBENCH_ARRAY(C_outputFromDsp), &t_DSP_start, &t_DSP_end);
    ...
    polybench_PM_print_cache(kernel_cache, clusterId, nthreads, t_CPU_sum / ROUND, t_DSP_sum / ROUND, t_DSP_cache_sum / ROUND, get_dataset_name());
