...
void conv3D(int ni, int nj, int nk, DATA_TYPE POLYBENCH_3D(A, NI, NJ, NK, ni, nj, nk),
...
void compareResults(int ni, int nj, int nk, DATA_TYPE POLYBENCH_3D(B, NI, NJ, NK, ni, nj, nk),
...
void convolution3DDSP(int ni, int nj, int nk, DATA_TYPE POLYBENCH_3D(A, NI, NJ, NK, ni, nj, nk),
                      DATA_TYPE POLYBENCH_3D(B, NI, NJ, NK, ni, nj, nk),
                      DATA_TYPE POLYBENCH_3D(B_outputFromDsp, NI, NJ, NK, ni, nj, nk), double *t_DSP_start,
                      double *t_DSP_end)
{
...
void convolution3DDSP_cache(int ni, int nj, int nk, DATA_TYPE POLYBENCH_3D(A, NI, NJ, NK, ni, nj, nk),
                            DATA_TYPE POLYBENCH_3D(B, NI, NJ, NK, ni, nj, nk),
                            DATA_TYPE POLYBENCH_3D(B_outputFromDsp, NI, NJ, NK, ni, nj, nk), double *t_DSP_start,
                            double *t_DSP_end)
{
    ...
    for (i = 1; i < _PB_NI - 1; ++i) {
        args[3] = i;
        hthread_group_exec(groupId, kernel_cache, 4, 2, args);
    }
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < ROUND; i++) {
        ...
        convolution3DDSP(ni, nj, nk, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(B_outputFromDsp),
                         &t_DSP_start, &t_DSP_end);
        t_DSP_sum += t_DSP_end - t_DSP_start;

        init(ni, nj, nk, POLYBENCH_ARRAY(A));
        convolution3DDSP_cache(ni, nj, nk, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(B_outputFromDsp),
                               &t_DSP_start, &t_DSP_end);
    ...
    polybench_PM_print_cache(kernel_cache, clusterId, nthreads, t_CPU_sum / ROUND, t_DSP_sum / ROUND,
