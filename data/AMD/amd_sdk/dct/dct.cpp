...
class DCT
{
    ...
    public:
         ...
         */
        DCT(int initWidth = 64, int initHeight = 64, unsigned int initBlockWidth = 8, 
        int initIterations = 1, unsigned int initInverse = 0, unsigned int seedValue = 123):
          seed(seedValue), width(initWidth), height(initHeight), 
          blockWidth(initBlockWidth), iterations(initIterations), inverse(initInverse)
...
void printHelp(const char* programName) {
    std::cout << "使用方法: " << programName << " [选项]\n"
              << "选项:\n"
              << "  -w <width>          设置输入数组的宽度 (默认: 64)\n"
              << "  -h <height>         设置输入数组的高度 (默认: 64)\n"
              << "  -b <blockWidth>     设置块的宽度 (默认: 8)\n"
              << "  -i <iterations>     设置迭代次数 (默认: 1)\n"
              << "  -inv                执行逆 DCT\n"
              << "  -s <seed>           设置随机种子 (默认: 123)\n"
              << "  --help              显示帮助信息\n";
}
...
int main(int argc, char* argv[]) {
    ...
    // 创建 DCT 对象，传递解析后的参数
    DCT hipDCT(width, height, blockWidth, iterations, inverse, seed);
